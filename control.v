...
// At the rising edge of the clock signal, if reset = 0, resets the state of the FSM to its initial state, the opcode line.
...
module control(clock, reset, data_bus, oper0, pc_mode, mar_mode, mux_select, mem_oe,
  ...
  input clock;
  input reset;
  input [31:0] data_bus;     // Modified to 32 bits
  input [31:0] oper0;        // Modified to 32 bits
  output [2:0] pc_mode;
  output [1:0] mar_mode;
  output mux_select;
  output mem_oe;
  ...
  // Assigning to 32-bit wide output signals
